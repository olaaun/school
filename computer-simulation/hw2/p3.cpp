...
int next_event_type, num_custs_delayed[3], num_delays_required, num_events,
    num_in_q[3], server_status[3], queue_limits[3];
float mean_interarrival, mean_service,
      sim_time, time_arrival[3][Q_LIMIT + 1], time_last_event, time_next_event[4],
      total_of_delays, total_of_system_delays[3];
FILE  *outfile;
...
main()  /* Main function. */
{
        ...
        int buffer_size[3][3] = {{0,INF, INF}, {0,INF, 7}, {0,7, INF}};
        /* Read input parameters. */
        fprintf(outfile, "Single-server queueing system\n\n");
        fprintf(outfile, "Mean service time%16.3f minutes\n\n", 1.0);
        fprintf(outfile, "Number of customers%14d\n\n", 100);
        for(int j = 0; j < 3; j++){
        memcpy(queue_limits, buffer_size[j], sizeof(queue_limits));
        fprintf(outfile, "\n\nBuffer size %14d and %14d \n",
                queue_limits[1], queue_limits[2]);
        for(int i = 0; i < 5; i++) {
                fprintf(outfile, "\n\n-----------------------------\n\n");
...
void depart(int queue)  /* Departure event function. */
{
        ...
        if (num_in_q[queue] == 0) {
                ...
                total_of_system_delays[queue] += delay + serv_t;
